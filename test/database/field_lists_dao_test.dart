...
void main() {
  ...
  test('watchWithEntriesCountByFieldId', () async {
    final entrysDao = EntrysDao(appDatabase);
    ...
    final entry1 = Entry(
      ...
      rank: 0,
      askedCount: BigInt.from(0),
      wronglyAnsweredCount: BigInt.from(0),
    );
    final entry2 = Entry(
      id: const Uuid().v4(),
      fieldListId: fieldList6.id,
    ...
    final result = await fieldListsDao
        .watchWithEntriesCountByFieldId(fieldId)
        .first;
    expect(result, [(fieldList6, 4), (fieldList4, 2), (fieldList1, 0)]);
    var gottenFieldList = result[0].$1;
    expect(gottenFieldList.id, fieldList6.id);
    expect(gottenFieldList.fieldId, fieldList6.fieldId);
    expect(gottenFieldList.name, fieldList6.name);
    expect(gottenFieldList.creationAt, fieldList6.creationAt);
    expect(gottenFieldList.lastModificationAt, fieldList6.lastModificationAt);
    expect(gottenFieldList.languageTag, fieldList6.languageTag);
    expect(gottenFieldList.checkType, fieldList6.checkType);
    expect(gottenFieldList.sortBy, fieldList6.sortBy);
    expect(gottenFieldList.doesReadAnswer, fieldList6.doesReadAnswer);
    expect(gottenFieldList.usageCount, fieldList6.usageCount);
    expect(gottenFieldList.color, fieldList6.color);
    expect(
      gottenFieldList.emulationNumberOfQuestions,
      fieldList6.emulationNumberOfQuestions,
    );
    expect(gottenFieldList.emulationDays, fieldList6.emulationDays);
    expect(
      gottenFieldList.testsReadingQuestionLetterDuration,
      fieldList6.testsReadingQuestionLetterDuration,
    );
    expect(
      gottenFieldList.testsFindingAnswerDuration,
      fieldList6.testsFindingAnswerDuration,
    );
    expect(
      gottenFieldList.testsTypingAnswerLetterDuration,
      fieldList6.testsTypingAnswerLetterDuration,
    );
    expect(
      gottenFieldList.studyTillCorrectReadingQuestionLetterDuration,
      fieldList6.studyTillCorrectReadingQuestionLetterDuration,
    );
    expect(
      gottenFieldList.studyTillCorrectFindingAnswerDuration,
      fieldList6.studyTillCorrectFindingAnswerDuration,
    );
    expect(
      gottenFieldList.studyTillCorrectTypingAnswerLetterDuration,
      fieldList6.studyTillCorrectTypingAnswerLetterDuration,
    );
    expect(
      gottenFieldList.testsTimeOfAnswerAction,
      fieldList6.testsTimeOfAnswerAction,
    );
    expect(
      gottenFieldList.doesObfuscateQuestion,
      fieldList6.doesObfuscateQuestion,
    );
  });
