...
void main() {
  ...
  group("Create wrongAnswer", () {
    ...
    test("Invalid wrongAnswer: sessionId is invalid UUID v4", () {
      ...
      expect(
        ...
        throwsA(
          predicate(
            (e) => e is InvalidDataException && e.message.contains("sessionId"),
          ),
        ),
      );
    });
    ...
    test("Invalid wrongAnswer: entryId is invalid UUID v4", () {
      ...
      expect(
        ...
        throwsA(
          predicate(
            (e) => e is InvalidDataException && e.message.contains("entryId"),
          ),
        ),
      );
    });
    ...
    test("Invalid wrongAnswer: value is an empty String", () {
      ...
      expect(
        ...
        throwsA(
          predicate((e) => e is SqliteException && e.message.contains("value")),
        ),
      );
        ...
        throwsA(
          predicate(
            (e) =>
                ...
                e.message.contains(
                  "creationAt is before the session creationAt",
                ),
          ),
        ),
      );
  ...
  group("Get by sessionId & entryId", () {
    test("There is at least one wrong answer", () async {
      ...
      List<WrongAnswer> wrongAnswers = await wrongAnswersDao
          .getBySessionIdAndEntryId(sessionId, entryId);
    ...
    test("There is no wrong answer", () async {
      ...
      await entrysDao.create(entry1.toCompanion(true));
      List<WrongAnswer> wrongAnswers = await wrongAnswersDao
          .getBySessionIdAndEntryId(sessionId1, entryId1);
  ...
  test("Delete by sessionId and entryId", () async {
    ...
    List<WrongAnswer> wrongAnswers = await wrongAnswersDao
        .getBySessionIdAndEntryId(sessionId, entryId);
    expect(wrongAnswers.length, 0);
    wrongAnswers = await wrongAnswersDao.getBySessionIdAndEntryId(
      sessionId1,
      entryId,
    );
