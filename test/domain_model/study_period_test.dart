...
main() {
  ...
  group("_id tests", () {
    test("_id is a valid UUID", () {
      expect(() => StudyPeriod("", fieldList, 0, 1, Duration()),
          throwsArgumentError);
      expect(() => StudyPeriod("weuwe", fieldList, 0, 1, Duration()),
          throwsArgumentError);
      expect(() => StudyPeriod(uuid.v4(), fieldList, 0, 1, Duration()),
          returnsNormally);
    });
    test("_id has been assigned the correct value", () {
      String uuidString = uuid.v4();
      final studyPeriod = StudyPeriod(uuidString, fieldList, 0, 1, Duration());
  ...
  group("_fieldList tests", () {
    test("_fieldList has been assigned the correct value", () {
      ...
      final studyPeriod = StudyPeriod(uuid.v4(), fieldList1, 0, 1, Duration());
  ...
  group("_currentQuestionCounter tests", () {
    test("_currentQuestionCounter cannot be negative", () {
      expect(() => StudyPeriod(uuid.v4(), fieldList, -1, 1, Duration()),
          throwsArgumentError);
      expect(() {
        final studyPeriod = StudyPeriod(uuid.v4(), fieldList, 0, 1, Duration());
    ...
    test(
        ...
        () {
      expect(() {
        final studyPeriod = StudyPeriod(uuid.v4(), fieldList, 3, 1, Duration());
  ...
  group("_triesNumber tests", () {
    test("_triesNumber cannot be smaller than one", () {
      expect(() => StudyPeriod(uuid.v4(), fieldList, 0, 0, Duration()),
          throwsArgumentError);
      expect(() => StudyPeriod(uuid.v4(), fieldList, 0, -1, Duration()),
          throwsArgumentError);
    });
    test("_triesNumber has been assigned the correct value", () {
      final studyPeriod = StudyPeriod(uuid.v4(), fieldList, 0, 3, Duration());
  ...
  group("_elapsedTime tests", () {
    test(
      ...
      expect(() {
        final studyPeriod =
            StudyPeriod(uuid.v4(), fieldList, 0, 3, Duration(seconds: 10));
        studyPeriod.elapsedTime = Duration();
      }, throwsArgumentError);
    });
