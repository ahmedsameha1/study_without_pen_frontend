...
class _EntriesState implements EntriesState {
  const _EntriesState({this.status = EntriesStatus.initial, this.entriesPageData, this.currentTabIndex = 0, final  List<TabData> tabs = const [TabData(status: TabDataStatus.loading, outdated: true, name: scoreTabName, description: scoreTabDescription, entries: <EntryEntity>[]), TabData(status: TabDataStatus.loading, outdated: true, name: strugglingTabName, description: strugglingTabDescription, entries: <EntryEntity>[])]}): _tabs = tabs;
